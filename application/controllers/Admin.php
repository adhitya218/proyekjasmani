...
class Admin extends CI_Controller{
  ...
  {
    $no_ident = $this->uri->segment(3);
    $where = array('no_identitas' => $no_ident);
  ...
  public function proses_tambah_user()
  {
    ...
    if($this->form_validation->run() == TRUE)
    {
      if($this->session->userdata('token_generate') === $this->input->post('token'))
      {
        ...
        $no_ident     = $this->input->post('no_identitas',TRUE);
  ...
  {
    $data['list_tingkatan'] = $this->M_admin->select('tb_tingkatan');
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'));
    $this->load->view('admin/form_barangmasuk/form_insert',$data);
  }

      ####### tabel_barang masuk ganti siswa
  public function tabel_siswa()
  {
    $data = array(
              'list_data' => $this->M_admin->select('tb_siswa'),
              'avatar'    => $this->M_admin->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'))
            );
    $this->load->view('admin/tabel/tabel_siswa',$data);
  }

  public function update_siswa($no_ak)
  {
    $where = array('no_ak' => $no_ak);
    $data['data_siswa'] = $this->M_admin->get_data('tb_siswa',$where);
    $data['list_tingkatan'] = $this->M_admin->select('tb_tingkatan');
    $data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'));
  ...
  public function proses_databarang_masuk_insert()
  {
    ...
    $this->form_validation->set_rules('no_ak','No_ak','required');


    if($this->form_validation->run() == TRUE)
    {
      $nama = $this->input->post('nama',TRUE);
      $no_ak      = $this->input->post('no_ak',TRUE);
      ...
      $this->M_admin->insert('tb_siswa',$data);
  ...
  public function proses_siswa_update()
  {
    $this->form_validation->set_rules('nama','Nama','required');
    ...
    if($this->form_validation->run() == TRUE)
    {
      ...
      $this->M_admin->update('tb_siswa',$data,$where);
      $this->session->set_flashdata('msg_berhasil','Data Barang Berhasil Diupdate');
      redirect(base_url('admin/tabel_siswa'));
    }else{
